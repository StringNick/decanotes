 ...
 */
export function useEditorState({ initialBlocks, onBlocksChange, config }: UseEditorStateProps): UseEditorStateReturn {
  // Core state
  ...
  const setBlocksImmediate = useCallback(
    (newBlocks: EditorBlock[]) => {
  ...
  const importFromMarkdown = useCallback(
    (markdown: string, markdownPlugins: MarkdownPlugin[]) => {
