 ...
 */
export function useEditorDragDrop({ blocks, blockPlugins, config, actions }: UseEditorDragDropProps) {
  ...
  const startDrag = useCallback(
    (blockId: string, event: React.MouseEvent | React.TouchEvent) => {
      ...
      if (files.length > 0) {
        files.forEach((file, fileIndex) => {
          if (file.type.startsWith('image/')) {
            ...
            const imageUrl =
              typeof URL !== 'undefined' && URL.createObjectURL ? URL.createObjectURL(file) : `file://${file.name}`; // Fallback for React Native
